...
| `20260805140000_notification_preferences.sql` | Per-category switches for the feed: `notification_preferences` (one row per profile, one boolean per category — `tours`, `matches`, `activity` — with own-row SELECT/INSERT/UPDATE policies and no DELETE, since deleting a row and switching everything back on are the same state). A missing row means all-on, so the table stays empty until someone opens the settings dialog and `NOTIFICATION_PREFERENCE_DEFAULTS` in `lib/services/notifications.ts` is the real initial state. **No email column**: nothing in the app sends email (the `saved-search-alerts` function below is inert), and a switch that does nothing is worse than no switch — add the column with the sender. The filter is applied at *read* time (`listMyNotifications` / `countMyUnreadNotifications` narrow on `kind`), never at write time, so muting a category hides its history rather than punching a hole in it. **Regenerate `lib/database.types.ts` after applying.** Applied 2026-08-05: table created empty. |
